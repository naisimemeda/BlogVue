  ...
  <div class="panel panel-default list-panel search-results">
    <div class="panel-heading">
      <h3 class="panel-title">
        ...
        <button v-for="(item, index) in filters" :key="index"
    ...
    <div class="panel-body">
      <div v-for="(result, index) in results" :key="index" class="result">
        ...
        <div class="info">
          ...
          <span class="date">
            {{ result.created_at}} ⋅
            <i class="fa fa-thumbs-o-up"></i> {{ result.like_count}} ⋅
            <i class="fa fa-comments-o"></i> {{ result.comment_count }}
          </span>
        </div>
    ...
    getArticlesByKeyword(keyword, filter) {
      if (keyword) {
        ...
        this.$store.commit('UPDATE_SEARCH_VALUE', keyword)
          register.Search(keyword, filter).then(response => {
            this.results = this.$store.getters.getArticlesByKeyword(keyword, response.data)
          })
        }
      }
  }
